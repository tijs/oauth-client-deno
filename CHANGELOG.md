...
- **CI**: Reconfigure npm trusted publisher.
...
- **`prompt` option in `AuthorizeOptions`**: Pass `prompt: "create"` to direct
  users to the PDS account registration page instead of login. After
  registration, the normal OAuth callback completes and the user is
  authenticated. This enables seamless "Create account" flows where apps can
  offer PDS selection and users are redirected back after signup.
